...
const fields = [
  ...
  { name: 'confirmPassword', type: 'password' as const, label: t('auth.resetPassword.confirmPassword'), required: true }
]
...
async function onSubmit(payload: FormSubmitEvent<Schema>) {
  ...
  try {
    const api = useApi()
    await api.auth.postResetPassword({ resetPasswordRequest: { token: token.value, newPassword: payload.data.password } })
    success.value = true
  } catch (err) {
    ...
    <div class="w-full max-w-sm flex flex-col gap-4">
      <template v-if="!token">
        <div class="flex flex-col items-center gap-4 text-center py-8">
          ...
          >{{ t('auth.resetPassword.requestNew') }}</ULink>
        </div>
      </template>
      ...
      <template v-else-if="success">
        <div class="flex flex-col items-center gap-4 text-center py-8">
          ...
          <ULink
            to="/auth/sign-in"
            class="text-primary font-medium"
          >{{ t('auth.resetPassword.success.signInLink') }}</ULink>
        </div>
      </template>
      ...
      <template v-else>
        ...
        <UAuthForm
          ...
          :submit="{ label: t('auth.resetPassword.submit'), loading, block: true }"
          @submit="onSubmit"
        >
          <template
            v-if="errorMessage"
